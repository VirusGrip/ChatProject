...
current_chat_type = 'global'  # Либо 'global', либо 'private'
current_private_chat_username = None  # Имя пользователя для приватного чата
# Цвета и шрифты
...
def open_emoji_picker(target_widget=None):
    ...
    # Окно выбора эмодзи
    emoji_window = QDialog(target_widget.parentWidget())  # Делаем его дочерним для приватного окна
    ...
    # Устанавливаем модальность окна на уровень приватного чата, чтобы он оставался на переднем плане
    emoji_window.setWindowModality(Qt.WindowModal)
    ...
    # После закрытия окна возвращаем приватное окно на передний план
    target_widget.parentWidget().raise_()
    target_widget.parentWidget().activateWindow()
...
def send_private_file(recipient_username):
    """Открывает диалог для выбора файла и отправляет его в приватный чат частями."""
    # Создаем диалог как дочерний элемент окна приватного чата
    file_dialog = QFileDialog(private_chat_windows[recipient_username]['window'], "Выберите файл")
    file_dialog.setWindowModality(Qt.WindowModal)  # Модальность для приватного окна, чтобы оно оставалось активным
    ...
    if file_path:
        ...
        file_url = create_file_link(file_name)
        html_link = f"<a href='{file_url}' style='color: {USER_COLOR}; text-decoration: none;'>{file_name}</a>"
        private_chat_windows[recipient_username]['text_edit'].append(f"Вы отправили файл: {html_link}")
        private_chat_windows[recipient_username]['text_edit'].setTextInteractionFlags(Qt.TextBrowserInteraction)
        private_chat_windows[recipient_username]['text_edit'].append("")

    # После закрытия окна выбора файла возвращаем приватное окно на передний план
    private_chat_windows[recipient_username]['window'].raise_()
    private_chat_windows[recipient_username]['window'].activateWindow()
...
def start_private_chat(username):
    ...
    # Показываем окно чата и поднимаем его на передний план
